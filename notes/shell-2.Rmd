...
#### list items, with not command line arguments (aka flags){.bs-callout .bs-callout-warning}
```{r}
ls
```
...
#### list with -F {.bs-callout .bs-callout-warning}
```{r}
ls -F
```

#### -a show hidden items {.bs-callout .bs-callout-danger}

```{r}
ls -Fa
```

#### -l to see detailed information {.bs-callout .bs-callout-note}

```{r}
ls -Fla
```

#### List contents, including one level below {.bs-callout .bs-callout-info}
...
#### List files recursively with -R {.bs-callout .bs-callout-danger}
...
#### cd into directory that has some of Nelle's data {.bs-callout .bs-callout-note}
...
#### list the files, with details {.bs-callout .bs-callout-warning}
...
#### The "goodiff" file {.bs-callout .bs-callout-danger}
...
#### head  {.bs-callout .bs-callout-warning}
```{r}
head NENE02040B.txt
```
...
#### tail {.bs-callout .bs-callout-danger}
```{r tail4}
tail NENE02040B.txt
```
...
#### more {.bs-callout .bs-callout-warning}
```{r}
more NENE02040B.txt
```
...
#### Here's my path {.bs-callout .bs-callout-info}
...
```
#### list the files {.bs-callout .bs-callout-info}
```{r}
ls -la
```

These are small files, we might as well look at one:
#### list the files {.bs-callout .bs-callout-warning}
```{r}
more cubane.pdb
```
...
### wc usages {.bs-callout .bs-callout-note}
```{r}
wc -l cubane.pdb
```
...
#### wc with "-l" to see only the number of lines {.bs-callout .bs-callout-note}
```{r}
wc -l *.pdb
```
...
#### wc piped to sort {.bs-callout .bs-callout-info}
```{r}
wc -l *.pdb | sort 
```
...
#### Save the sorted list into a file "records.txt" {.bs-callout .bs-callout-info}
```{r}
...
#### grep using a regular expression {.bs-callout .bs-callout-info}
...
#### Change the working directory {.bs-callout .bs-callout-note}
...
#### Inspect the gyre data {.bs-callout .bs-callout-note}
